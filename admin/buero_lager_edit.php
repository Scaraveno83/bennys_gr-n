...
/* === Produktliste (alphabetisch sortiert, identisch zu Haupt-/Azubi-Lager) === */
$produkteListe = [
...
if (isset($_GET['delete'])) {
  $id = (int)$_GET['delete'];
  $pdo->prepare("DELETE FROM buero_lager_verlauf WHERE id = ?")->execute([$id]);
  header("Location: buerolager_edit.php");
  exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
  ...
  if ($produkt && $menge > 0) {
    ...
    // Wenn Produkt noch nicht im Lager existiert → anlegen
    $check = $pdo->prepare("SELECT COUNT(*) FROM buero_lager WHERE produkt = ?");
    $check->execute([$produkt]);
    if ($check->fetchColumn() == 0) {
      $pdo->prepare("INSERT INTO buero_lager (produkt, bestand) VALUES (?, 0)")
          ->execute([$produkt]);
    }

    // Bestand anpassen
    if ($aktion === 'hinzugefügt') {
      $pdo->prepare("UPDATE buero_lager SET bestand = bestand + ? WHERE produkt = ?")
          ->execute([$menge, $produkt]);
    } else {
      $pdo->prepare("UPDATE buero_lager SET bestand = GREATEST(bestand - ?, 0) WHERE produkt = ?")
          ->execute([$menge, $produkt]);
    }
  }
...
<head>
<meta charset="UTF-8">
<title>📁 Bürolager verwalten | Admin</title>
