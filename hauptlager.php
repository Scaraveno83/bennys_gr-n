...
$watchlistCandidates = array_filter(
  $bestand,
  fn($menge) => $menge <= max($lowStockThreshold * 1.5, $lowStockThreshold + 25)
);
...
<main class="inventory-page">
  <section class="inventory-hero">
    <div class="inventory-hero__content">
      <span class="inventory-hero__tag">Zentrales Lager</span>
      <h1 class="inventory-hero__title">🏭 Hauptlager</h1>
      <p class="inventory-hero__lead">
        Modernes Control-Center für alle Materialbewegungen im Stammwerk. Transparente Kennzahlen,
        schnelle Filter und ein fokussierter Workflow halten das Hauptlager schlank und übersichtlich.
      </p>
      ...
      <div class="inventory-hero__actions">
        <a class="inventory-cta" href="#lageraktion">Neue Buchung erfassen</a>
        <span class="inventory-hero__hint">Suche, Filter und Watchlist helfen beim Priorisieren der Bestände.</span>
      </div>
    </div>
  </section>
  ...
  <section class="metric-deck" aria-label="Kennzahlen">
    <article class="metric-card metric-card--accent">
      ...
      <div class="metric-card__values">
        <span class="metric-card__label">Produktarten</span>
        <span class="metric-card__value"><?= number_format($anzahlProdukte, 0, ',', '.') ?></span>
      </div>
      <span class="metric-card__foot">Aktiv im Hauptlager</span>
    </article>
    ...
    <article class="metric-card">
      <span class="metric-card__icon" aria-hidden="true">🧮</span>
      <div class="metric-card__values">
        <span class="metric-card__label">Nutzeraktivität</span>
        <span class="metric-card__value"><?= number_format(count($timelineEntries), 0, ',', '.') ?></span>
      </div>
      <span class="metric-card__foot">Aktionen der letzten Sessions</span>
    </article>
  </section>
    ...
    <article class="inventory-panel inventory-panel--table" data-inventory>
      <header class="panel-header">
        <div class="panel-titles">
          <h2>Live-Bestände</h2>
          <p>Visualisiere das Sortiment, markiere Engpässe und finde Artikel sekundenschnell.</p>
        </div>
    ...
    <aside class="inventory-sidebar">
      <article class="inventory-panel inventory-panel--insights">
        <header class="panel-header">
          <div class="panel-titles">
            <h2>Watchlist</h2>
            <p>Fokussiere dich auf knappe Artikel, bevor sie kritisch werden.</p>
          </div>
        </header>
        ...
        <?php else: ?>
          <p class="watchlist-empty">Aktuell sind alle Artikel komfortabel versorgt.</p>
        <?php endif; ?>
      </article>
      ...
      <article class="inventory-panel inventory-panel--form" id="lageraktion">
        <header class="panel-header">
          <div class="panel-titles">
            ...
            <p>Jede Buchung wird automatisch protokolliert und in Echtzeit visualisiert.</p>
          </div>
        </header>
          ...
          <div class="form-columns">
            ...
            <div class="input-control">
              ...
              <div class="segmented-control">
                <input type="radio" id="aktion-add-haupt" name="aktion" value="hinzugefügt" checked>
                <label for="aktion-add-haupt">Hinzufügen</label>
                <input type="radio" id="aktion-remove-haupt" name="aktion" value="entnommen">
                <label for="aktion-remove-haupt">Entnehmen</label>
              </div>
            </div>
          </div>
      ...
      <article class="inventory-panel inventory-panel--full inventory-panel--history">
        <header class="panel-header">
          <div class="panel-titles">
            ...
            <p>Die letzten <?= number_format(count($timelineEntries), 0, ',', '.') ?> Buchungen für Administratoren.</p>
          </div>
        </header>
